...
class EtaTransformer extends Transformer
{
    ...
    private function calculateRideFares($distance_in_unit, $dropoff_time_in_seconds, $zone_type, $type_prices, $coupon_detail)
    {
        ...
        $service_fee = $zone_type->admin_commission;

        if(($zone_type->admin_commission_type) == 1)
        {
            Log::info("inside");
            $service_fee  = ($subtotal_price  * $service_fee / 100);
        }

        Log::info($service_fee);

        /*get admin commision From zonetype */
        // Log::info("admin_commission");
        // Log::info($zone_type->admin_commission);
        // Log::info("service_tax");
        // Log::info($zone_type->service_tax);
        // Log::info("admin_commission_type");
        // Log::info($zone_type->admin_commission_type);

        // Admin commision
        $without_discount_admin_commision = (($subtotal_price + $discount_amount) + $service_fee);
        ...
        $discount_admin_commision = ($coupon_applied_sub_total + $service_fee);
